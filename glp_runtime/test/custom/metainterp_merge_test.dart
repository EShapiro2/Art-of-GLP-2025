...
void main() {
  test('Metainterpreter with merge object program', () {
    ...
    final prog = BC.prog([
      ...
      BC.COMMIT(),
      BC.PROCEED(),
      BC.CLAUSE_NEXT('clause/2_c2'),  // If clause 1 fails, try clause 2
      ...
      BC.COMMIT(),
      BC.PROCEED(),
      BC.CLAUSE_NEXT('clause/2_c3'),  // If clause 2 fails, try clause 3
    ...
    const varListA = 10;  // Single ID for [a] variable
    rt.heap.addWriter(WriterCell(varListA, varListA));  // Same ID
    rt.heap.addReader(ReaderCell(varListA));
    rt.heap.bindWriterStruct(varListA, '[|]', [ConstTerm('a'), ConstTerm('[]')]);

    // Build the list [b]
    const varListB = 12;  // Single ID for [b] variable
    rt.heap.addWriter(WriterCell(varListB, varListB));  // Same ID
    rt.heap.addReader(ReaderCell(varListB));
    rt.heap.bindWriterStruct(varListB, '[|]', [ConstTerm('b'), ConstTerm('[]')]);

    // Build Zs (unbound writer for result)
    const varZs = 14;  // Single ID for Zs variable
    rt.heap.addWriter(WriterCell(varZs, varZs));  // Same ID
    rt.heap.addReader(ReaderCell(varZs));

    // Build merge([a],[b],Zs)
    const varMerge = 16;  // Single ID for merge variable
    rt.heap.addWriter(WriterCell(varMerge, varMerge));  // Same ID
    rt.heap.addReader(ReaderCell(varMerge));
    rt.heap.bindWriterStruct(varMerge, 'merge', [
      VarRef(varListA, isReader: false),
      VarRef(varListB, isReader: false),
      VarRef(varZs, isReader: false),
    ]);

    print('STRUCTURES:');
    print('  [a] = writer $varListA bound to [|](a, [])');
    print('  [b] = writer $varListB bound to [|](b, [])');
    print('  Zs = writer $varZs (unbound)');
    print('  merge([a],[b],Zs) = reader $varMerge');
    print('');

    // Start goal: run(merge([a],[b],Zs))
    const goalId = 100;
    final env = CallEnv(readers: {0: varMerge});
    ...
    print('RESULTS');
    print('=' * 70);
    print('Zs (Var$varZs) bound: ${rt.heap.isWriterBound(varZs)}');

    expect(rt.heap.isWriterBound(varZs), true, reason: 'Zs should be bound');
