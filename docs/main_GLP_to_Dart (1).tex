...
\mypara{Arithmetic Expressions}
GLP provides arithmetic expressions for numeric computation. Arithmetic expressions are terms that evaluate to integer values using the \verb|evaluate/2| system predicate:

\textbf{Syntax}: Arithmetic expressions support standard infix notation with precedence:
\begin{verbatim}
expr ::= number
       | variable
       | '-' expr                % unary negation
...
\item Multiplicative: \verb|*|, \verb|/|, \verb|mod|
\item Additive: \verb|+|, \verb|-|
\end{enumerate}
All operators are left-associative. Unary negation \verb|-X| uses functor \verb|neg| to avoid ambiguity with binary subtraction.

\textbf{Semantics}: Arithmetic expressions are \emph{not guards}—they have side effects via suspension. The parser transforms infix notation to prefix structure notation: \verb|X + Y| becomes \verb|+(X, Y)|. Evaluation uses the system predicate \verb|evaluate(Expr?, Result)| which follows three-valued semantics: success (all operands bound integers), suspend (unbound reader operand), or fail (non-integer operand).

\textbf{Example}:
\begin{verbatim}
add(X, Y, Z) :- execute('evaluate', [X? + Y?, Z]).
% Query: add(3, 4, Z) results in Z = 7
% Parser transforms: X? + Y? → +(VarRef(X,true), VarRef(Y,true))
\end{verbatim}
...
GLP distinguishes between \emph{guards} (three-valued) and \emph{system predicates called via execute} (two-valued):

\textbf{Guards} are pure tests with \emph{three-valued semantics} (success/suspend/fail). They appear after clause heads separated by \verb=|= and enable conditional clause selection. Guards are \emph{patient}—they suspend on unbound variables rather than fail. Currently implemented: \verb|ground(X)| tests if \verb|X| contains no unbound variables; \verb|known(X)| tests if \verb|X| is bound; \verb|otherwise| succeeds if previous clauses failed (not suspended). Planned: type guards (\verb|number(X)|, \verb|integer(X)|, \verb|writer(X)|, \verb|reader(X)|), comparison guards (\verb|X < Y|, \verb|X =< Y|, \verb|X > Y|, \verb|X >= Y|, \verb|X =:= Y|, \verb|X =\= Y|), unification guards (\verb|X = Y|, \verb|X \= Y|), and control guards (\verb|true|). The \verb|ground(X)| guard allows relaxing the single-reader constraint for \verb|X?| in that clause, as multiple occurrences of a ground term do not violate single-writer.

\textbf{System predicates called via execute} have \emph{two-valued semantics} (success/abort). They execute synchronously during the BODY phase via \verb|execute('predicate_name', [Args])|. Unlike guards, they \emph{require all inputs bound}—unbound readers cause runtime abort. Examples: \verb|write(X)| for output, \verb|evaluate(Expr?, Result)| for arithmetic evaluation (aborts on unbound operands), \verb|current_time(T)| for timestamps, \verb|file_read(Path, Contents)| for file I/O.

\textbf{Safe Pattern}: Use guards to ensure preconditions before execute:
\begin{verbatim}
safe_divide(X, Y, Z) :-
...
Their full specification appears in Appendix~\ref{appendix:guards-system}.
...
This section tracks the current state of the GLP Dart implementation, identifying completed components, work in progress, and future development priorities.

\subsection{Completed Components}

\mypara{Core Runtime System}
\begin{itemize}
\item \textbf{Heap Management} (\texttt{lib/runtime/heap.dart}): Complete implementation of writer/reader cell pairs with proper dereferencing, binding, and SRSW enforcement. Supports both constant and structure terms.
\item \textbf{Goal Queue} (\texttt{lib/runtime/goal\_queue.dart}): FIFO scheduling with deterministic goal activation order.
\item \textbf{ROQ (Read-Only Queue)} (\texttt{lib/runtime/roq.dart}): Suspension management with deterministic FIFO wake order when variables are bound.
\item \textbf{Terms and Cells} (\texttt{lib/runtime/terms.dart}, \texttt{cells.dart}): Complete term algebra including constants, structures, lists, and reader/writer variables.
\item \textbf{Fairness Control} (\texttt{lib/runtime/fairness.dart}): Tail recursion budget (26 iterations) before yielding to event loop.
\end{itemize}

\mypara{Bytecode Infrastructure}
\begin{itemize}
\item \textbf{Opcode Definitions} (\texttt{lib/bytecode/opcodes.dart}): Core instruction set defined with Phase 1/Phase 2 separation.
\item \textbf{Bytecode Assembler} (\texttt{lib/bytecode/asm.dart}): Helper functions for bytecode generation (BC class).
\item \textbf{Runner Framework} (\texttt{lib/bytecode/runner.dart}): Basic execution context and runner structure.
\end{itemize}

\mypara{Testing Infrastructure}
\begin{itemize}
\item \textbf{Unit Tests}: Comprehensive tests for heap operations, suspension/wake mechanics, writer/reader binding.
\item \textbf{Integration Tests}: Basic merge, list operations, and simple metainterpreter scenarios.
\item \textbf{Test Programs}: Hand-compiled bytecode for merge, boot, p/q examples.
\end{itemize}

\subsection{Work in Progress}

\mypara{Bytecode Runner Completion}
\begin{itemize}
\item \textbf{Missing Instructions} (Priority: High): 
  \begin{itemize}
  \item Structure processing: \texttt{put\_structure}, \texttt{get\_structure}, \texttt{unify\_*} operations
  \item Environment management: \texttt{allocate}, \texttt{deallocate} for permanent variables
  \item Control flow: \texttt{call}, \texttt{execute}, \texttt{proceed}
  \item Guard predicates: \texttt{ground}, \texttt{known}, \texttt{otherwise}
  \end{itemize}
\item \textbf{Phase Separation}: Proper tentative binding ($\hat{\sigma}_w$) management during Phase 1.
\item \textbf{Commit Operation}: Atomic application of tentative bindings with ROQ processing.
\end{itemize}

\mypara{Module System}
\begin{itemize}
\item \textbf{Design Decision Needed}: Static vs dynamic module loading
\item \textbf{Namespace Management}: Predicate visibility and qualification rules
\item \textbf{Standard Library}: Core predicates (lists, arithmetic, meta)
\end{itemize}
...
\mypara{Compiler}
\begin{itemize}
\item \textbf{Parser}: GLP syntax to AST (target: 3-4 weeks)
\item \textbf{Analyzer}: SRSW verification, variable classification (target: 2 weeks)
\item \textbf{Code Generator}: AST to bytecode following v2.16 specification (target: 3 weeks)
\item \textbf{Optimization}: Initially none; later consider indexing, clause selection
\end{itemize}

\mypara{Metaprogramming Support}
\begin{itemize}
\item \textbf{reduce/3 Predicate}: Core metainterpreter primitive
\item \textbf{Standard Metainterpreters}: Debug, trace, control versions from paper
\item \textbf{Interactive Development}: REPL with incremental compilation
\end{itemize}

\mypara{Multiagent Extensions}
\begin{itemize}
\item \textbf{Agent Isolation}: Separate heaps and goal queues per agent
\item \textbf{Message Passing}: Communicate transaction implementation
\item \textbf{Network Protocol}: Cold calls and channel establishment
\item \textbf{Security}: Attestation, signing, encryption (deferred to Phase 2)
\end{itemize}
...
\hline
Heap Operations & 95\% & Complete \\
Goal Queue & 90\% & Complete \\
ROQ/Suspension & 85\% & Complete \\
Terms/Unification & 80\% & Complete \\
Bytecode Runner & 40\% & In Progress \\
Compiler & 0\% & Not Started \\
Metainterpreters & 15\% & Basic Tests Only \\
\hline
\end{tabular}
\end{center}
...
\mypara{Phase 1: Core VM Completion (2-3 weeks)}
\begin{enumerate}
\item Implement remaining bytecode instructions
\item Add conformance tests for each instruction
\item Verify paper examples can execute
\end{enumerate}

\mypara{Phase 2: Module System (1-2 weeks)}
\begin{enumerate}
\item Design module representation
\item Implement loading and linking
\item Create standard library modules
\end{enumerate}

\mypara{Phase 3: Compiler v1 (3-4 weeks)}
\begin{enumerate}
\item Build parser for GLP syntax
\item Implement SRSW analyzer
\item Generate bytecode from AST
\end{enumerate}

\mypara{Phase 4: Metaprogramming (2-3 weeks)}
\begin{enumerate}
\item Implement reduce/3 in bytecode
\item Port metainterpreters from paper
\item Build interactive development tools
\end{enumerate}

\mypara{Phase 5: Documentation and Examples (ongoing)}
\begin{enumerate}
\item Complete API documentation
\item Write tutorial with examples
...
\begin{itemize}
\item \textbf{Performance}: Current implementation prioritizes correctness over speed
\item \textbf{Memory}: No heap compaction; relies on Dart GC
\item \textbf{Debugging}: Limited visibility into suspension reasons
\item \textbf{Error Messages}: Basic error reporting needs improvement
\item \textbf{Module System}: Design decisions pending
\item \textbf{Standard Library}: Minimal built-in predicates
\end{itemize}
...
Guards provide read-only access to the runtime state of GLP computation with \emph{three-valued semantics} (success/suspend/fail). A guard appears after the clause head, separated by \verb=|=, and must be satisfied for the clause to be selected. Guards are \emph{patient}—they suspend on unbound variables rather than fail.

\textbf{Currently Implemented Guards}:
\begin{itemize}
\item \verb|ground(X)| succeeds if \verb|X| contains no unbound variables, suspends on unbound readers. With this guard, the clause body may contain multiple occurrences of \verb|X?| without violating the single-writer requirement, enabling safe replication of ground terms to multiple concurrent consumers.
\item \verb|known(X)| succeeds if \verb|X| is not a variable (though it may not be ground), suspends on unbound readers.
\item \verb|otherwise| succeeds if all previous clauses for this procedure failed (not suspended).
\end{itemize}

\textbf{Planned Type Guards}:
\begin{itemize}
\item \verb|number(X)| succeeds if \verb|X| bound to number, suspends if unbound, fails if bound to non-number
\item \verb|integer(X)| succeeds if \verb|X| bound to integer, suspends if unbound, fails otherwise
\item \verb|writer(X)| succeeds if \verb|X| is a writer variable, fails if reader or ground
\item \verb|reader(X)| succeeds if \verb|X| is a reader variable, fails if writer or ground
\end{itemize}

\textbf{Planned Arithmetic Comparison Guards} (require parser extension for infix syntax):
\begin{itemize}
\item \verb|X < Y| less than (suspends if either unbound, fails on type error)
\item \verb|X =< Y| less than or equal (note: \verb|=<| not \verb|<=| per Prolog convention)
\item \verb|X > Y| greater than
\item \verb|X >= Y| greater than or equal
\item \verb|X =:= Y| arithmetic equality (evaluates expressions, compares results)
\item \verb|X =\= Y| arithmetic inequality (evaluates expressions, compares results)
\end{itemize}

\textbf{Planned Unification Guards}:
\begin{itemize}
\item \verb|X = Y| unification guard (suspends on unbound readers, fails if cannot unify)
\item \verb|X \= Y| non-unification guard (succeeds if cannot unify, suspends on unbound readers)
\end{itemize}

\textbf{Planned Control Guards}:
\begin{itemize}
\item \verb|true| always succeeds (equivalent to no guard)
\end{itemize}

\textbf{Guard Syntax and Semantics}:

Comparison guards use infix syntax in source code and are transformed to prefix by the parser:
\begin{itemize}
\item Source: \verb|X < Y| $\rightarrow$ AST: \verb|Atom('<', [X, Y])| $\rightarrow$ Bytecode: \verb|guard_less|
\item Source: \verb|X =< Y| $\rightarrow$ AST: \verb|Atom('=<', [X, Y])| $\rightarrow$ Bytecode: \verb|guard_less_equal|
\item Source: \verb|X > Y| $\rightarrow$ AST: \verb|Atom('>', [X, Y])| $\rightarrow$ Bytecode: \verb|guard_greater|
\item Source: \verb|X >= Y| $\rightarrow$ AST: \verb|Atom('>=', [X, Y])| $\rightarrow$ Bytecode: \verb|guard_greater_equal|
\item Source: \verb|X =:= Y| $\rightarrow$ AST: \verb|Atom('=:=', [X, Y])| $\rightarrow$ Bytecode: \verb|guard_arith_equal|
\item Source: \verb|X =\= Y| $\rightarrow$ AST: \verb|Atom('=\\=', [X, Y])| $\rightarrow$ Bytecode: \verb|guard_arith_not_equal|
\end{itemize}
...
\textbf{Implementation Requirements}: Lexer must recognize multi-character operators (\verb|=<|, \verb|>=|, \verb|=:=|, \verb|=\=|, \verb|\=|). Parser must handle infix syntax in guard position and transform to prefix. Comparison operators are non-associative (expressions like \verb|X < Y < Z| are rejected).

\textbf{Example with Comparison Guards}:
\begin{verbatim}
...
\mypara{System predicates called via execute}
System predicates execute atomically with goal/clause reduction via \verb|execute('predicate_name', [Args])| and have \emph{two-valued semantics} (success/abort). Unlike guards, they \emph{require all inputs bound}—unbound readers cause runtime abort.

\textbf{Standard System Predicates}:
\begin{itemize}
\item \verb|evaluate(Expr?,Result)| evaluates arithmetic expressions (aborts on unbound operands)
\item \verb|write(X)|, \verb|nl()|, \verb|read(X)| for terminal I/O
\item \verb|file_read(Path,Contents)|, \verb|file_write(Path,Contents)| for file I/O
...
\item \verb|copy_term(X,Y)|, \verb|distribute_stream(X,Y)| for term operations
\end{itemize}

\mypara{Arithmetic Evaluation System Predicate}
The \verb|evaluate/2| system predicate provides arithmetic computation:

\textbf{Syntax}: \verb|execute('evaluate', [Expression, Result])|

\textbf{Expression Format}: Prefix structure notation with functors \verb|+|, \verb|-|, \verb|*|, \verb|/|, \verb|mod|, and \verb|neg| for unary negation. The parser automatically transforms infix expressions to this format.

\textbf{Two-valued semantics} (success/abort):
\begin{enumerate}
\item \textbf{Success}: All operands bound to numbers → compute result, unify with Result
\item \textbf{Abort}: Any operand unbound OR non-numeric OR division by zero → runtime abort
\end{enumerate}

\textbf{Type System}: Integers and floats supported. The lexer parses both \verb|int| and \verb|double| literals. Division by zero aborts.

\textbf{Example GLP Source}:
\begin{verbatim}
add(X, Y, Z) :- execute('evaluate', [X? + Y?, Z]).
compute(Z) :- execute('evaluate', [(2 + 3) * 4, Z]).
\end{verbatim}

\textbf{Compiles to bytecode}:
\begin{verbatim}
execute('evaluate', [+(VarRef(X, isReader:true),
                       VarRef(Y, isReader:true)),
                     VarRef(Z, isReader:false)])
\end{verbatim}

\textbf{Parser Transformation Rules}:
\begin{itemize}
\item \verb|X + Y| → \verb|+(X, Y)|
\item \verb|X - Y| → \verb|-(X, Y)|
\item \verb|X * Y| → \verb|*(X, Y)|
\item \verb|X / Y| → \verb|/(X, Y)|
\item \verb|X mod Y| → \verb|mod(X, Y)|
\item \verb|-X| → \verb|neg(X)|  (unary negation)
\item \verb|X + Y * Z| → \verb|+(X, *(Y, Z))|  (precedence)
\item \verb|(X + Y) * Z| → \verb|*(+(X, Y), Z)|  (parentheses)
\end{itemize}

\mypara{Arithmetic evaluation in assignments}
Arithmetic expressions can be used in assignment notation:
\begin{verbatim}
X? := E :- ground(E) | evaluate(E?,X).
\end{verbatim}
This ensures the expression is ground before calling the evaluator, maintaining program safety whilst providing convenient notation for mathematical computations.
