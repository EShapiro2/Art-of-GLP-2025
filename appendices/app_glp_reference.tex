...
\item[\texttt{known(X)}] Succeeds if \verb|X| is bound to a value (though that value may contain unbound variables in subterms). Fails if \verb|X| is unbound.
...
\item[\texttt{unknown(X)}] Succeeds if \verb|X| is an unbound variable. Fails if \verb|X| is bound to any value.
...
Type guards: \verb|ground|, \verb|known|, \verb|unknown|, \verb|integer|, \verb|number|, \verb|atom|, \verb|string|, \verb|constant|, \verb|compound|, \verb|tuple|, \verb|list|, \verb|is_list|. Equality guard: \verb|=?=|.
