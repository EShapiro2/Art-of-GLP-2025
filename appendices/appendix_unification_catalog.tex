...
\hline
$X =?= Y$ & succeeds if both ground and equal & No binding, pure test & §\ref{sec:recursive-structures} \\
$\sim(X =?= Y)$ & succeeds if both ground and different & Negated equality & §\ref{sec:recursive-structures} \\
\end{tabular}
\end{center}

Note: Unification \verb|X = Y| in guard position is not supported; use head unification instead.
...
\texttt{is\_list(X)} & succeeds if proper list & suspends if $X?$ unbound & §\ref{sec:flatten} \\
\texttt{known(X)} & succeeds if principal functor known & suspends if $X?$ unbound & §\ref{sec:glp-plain-meta} \\
\texttt{unknown(X)} & succeeds if $X$ unbound & fails if functor known & §\ref{sec:glp-plain-meta} \\
\texttt{atomic(X)} & succeeds if atomic & suspends if $X?$ unbound & §\ref{sec:recursive-numeric} \\
\texttt{compound(X)} & succeeds if compound & suspends if $X?$ unbound & §\ref{sec:recursive-structures} \\
\end{tabular}
\end{center}

\subsection{Arithmetic Comparison Guards}

\begin{center}
\begin{tabular}{llll}
\textbf{Guard} & \textbf{Behavior} & \textbf{On Reader} & \textbf{First Use} \\
\hline
\texttt{X >= Y} & succeeds if $X \geq Y$ & suspends if either unbound & §\ref{sec:integer-arithmetic} \\
\texttt{X > Y} & succeeds if $X > Y$ & suspends if either unbound & §\ref{sec:integer-arithmetic} \\
\texttt{X < Y} & succeeds if $X < Y$ & suspends if either unbound & §\ref{sec:integer-arithmetic} \\
\texttt{X =< Y} & succeeds if $X \leq Y$ & suspends if either unbound & §\ref{sec:integer-arithmetic} \\
\end{tabular}
\end{center}

Arithmetic comparison guards compare numeric values. When applied to readers, they suspend until the readers are bound to ground numeric values.

\subsection{Guard Negation}

\begin{center}
\begin{tabular}{lll}
\textbf{Guard} & \textbf{Negated Form} & \textbf{Behavior} \\
\hline
\texttt{integer(X?)} & \texttt{\~{}integer(X?)} & succeeds if X not an integer \\
\texttt{ground(X?)} & \texttt{\~{}ground(X?)} & succeeds if X not ground \\
\texttt{X? =?= Y?} & \texttt{\~{}(X? =?= Y?)} & succeeds if X and Y different \\
\end{tabular}
\end{center}

Negation inverts success/fail while preserving suspension. Arithmetic guards cannot be negated.
...
\item[flatten] §\ref{sec:flatten}: difference list threading, type-testing guards
\item[merge] §\ref{sec:streams-merge}: stream interleaving, tail mode selection
\item[meta-interpreter] §\ref{sec:glp-plain-meta}: known/unknown guards, program encoding
...
\item[split] §\ref{sec:streams-merge}: alternating element distribution
\end{description}
