...
\begin{definition}[Writer Assignment, Reader Assignment, Induced Reader Assignment, Suspension Set, Writer MGU]\label{definition:GLP-unification}

A substitution $\sigma$ is a \temph{writer assignment} if  $\sigma$:
\begin{enumerate}
...
\end{enumerate}
A substitution $\sigma$ is a \temph{reader assignment} if $V_\sigma \subset V?$.
If $\sigma$ is a writer assignment then its \temph{induced reader assignment} is the reader assignment  $\sigma?$  defined by $X?\sigma? = X\sigma$ for every $X\in V_\sigma$.
...
The occurs check condition for the induced reader assignment ensures that no writer is bound to a term containing its paired reader, preventing the formation of circular terms, as proven in Proposition~\ref{proposition:acyclicity}.
\end{remark}
