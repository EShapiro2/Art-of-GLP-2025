...
The \verb|copy| predicate dispatches on the type of term: strings and numbers are copied directly (the type guard permits multiple reader occurrences per Formal Box~\ref{formal:guards-multiple-reads}), while compound terms are decomposed using \verb|=..| (univ), their arguments copied recursively, and the results reassembled.

The key insight is that no explicit handling of unbound variables is needed. If a subterm is unbound, none of the guards (\verb|string(X?)|, \verb|number(X?)|, \verb|compound(X?)|) succeed, so the goal suspends. When the producer later binds the subterm, the appropriate guard succeeds and copying proceeds. This is GLP's natural suspension behavior at work.
...
When bob's clause head \verb|response(bob, 25)| unifies with alice's \verb|response(Name, Age)|, the writers \verb|Name| and \verb|Age| receive the constants \verb|bob| and \verb|25|. The induced reader assignment propagates these values to alice's waiting guards \verb|string(Name?)| and \verb|number(Age?)|.
...
alice([msg(hello, Reply)|Rest]) :-
    string(Reply?) |    % Waits for Reply before sending
    process(Reply?),
    alice(Rest?).
\end{verbatim}
