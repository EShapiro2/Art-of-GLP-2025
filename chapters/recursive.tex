...
The guard \verb|~(Xs? =?= [])| is a negated equality test that succeeds when \verb|Xs| is bound to something other than the empty list, suspends when \verb|Xs| is unbound, and fails when \verb|Xs| is the empty list.
...
The \verb|copy| predicate dispatches on the type of term: constants are copied directly (the type guard permits multiple reader occurrences per Formal Box~\ref{formal:guards-multiple-reads}), while compound terms are decomposed using \verb|=..| (univ), their arguments copied recursively, and the results reassembled.

The key insight is that no explicit handling of unbound variables is needed. If a subterm is unbound, none of the guards (\verb|constant(X?)|, \verb|compound(X?)|) succeed, so the goal suspends. When the producer later binds the subterm, the appropriate guard succeeds and copying proceeds. This is GLP's natural suspension behavior at work.
...
observe(A?, B, Log?) :- compound(B?) |
    B? =.. [F|ArgsB],
    observe_list(ArgsB?, ArgsA, ArgsLog),
    A =.. [F?|ArgsA?],
    Log =.. [F?|ArgsLog?].
...
When bob's clause head \verb|response(bob, 25)| unifies with alice's \verb|response(Name, Age)|, the writers \verb|Name| and \verb|Age| receive the constants \verb|bob| and \verb|25|. The induced reader assignment propagates these values to alice's waiting guards \verb|constant(Name?)| and \verb|constant(Age?)|.
...
alice([msg(hello, Reply)|Rest]) :-
    constant(Reply?) |    % Waits for Reply before sending
    process(Reply?),
    alice(Rest?).
\end{verbatim}
...
The \verb|substitute| predicate replaces all occurrences of \verb|X| with \verb|Y| in a tree. The \verb|replace| helper handles individual elements: if the element equals \verb|X|, replace with \verb|Y|; otherwise keep the original. The guard \verb|~(X? =?= Z?)| is a negated equality test that succeeds when the two values are different, suspends if either is unbound, and fails if they are equal.
...
The inner product computation assumes the two vectors have equal length. If they do not, the goal fails---neither the base case (one list non-empty) nor the recursive case (one list empty) applies. The \verb|otherwise| guard can catch this:

\begin{verbatim}
inner_product(Xs, Ys, unequal_vectors(Xs?, Ys?)) :- otherwise | true.
\end{verbatim}

This returns a structured error term rather than silent failure. Whether to include such a clause depends on the application: if callers guarantee equal-length inputs, it is unnecessary.

Several list operations share this equal-length requirement, including \verb|zip| which pairs corresponding elements. General techniques for error handling, including failsafe meta-interpreters that catch failures and report their context, are discussed in Section~\ref{sec:meta}.
%% Note: sec:meta is the Metaprogramming section in basic_concurrent.tex
...
\item \textbf{Delete.} Implement \verb|delete(List,X,Result)| removing all occurrences of \verb|X| from \verb|List|. Use the negated equality guard \verb|~(X? =?= Z?)|.
