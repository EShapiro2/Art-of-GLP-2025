...
\mypara{Module Awareness}
Meta-interpreters need to know which module's clauses to interpret. Each meta-interpreter takes a module parameter \verb|M| and accesses clauses via \verb|M # reduce(Goal, Body)|. Cross-module calls \verb|M1 # G| are handled by switching to module \verb|M1| for the goal \verb|G|.
...
run(M, M1 # G) :- run(M1?, G?).                  % cross-module RPC
...
run(M, true, []).                                % halt
run(M, (A,B), Zs?) :-                            % fork
    run(M?, A?, Xs), run(M?, B?, Ys),
    merge(Xs?, Ys?, Zs).
run(M, M1 # G, Xs?) :- run(M1?, G?, Xs).         % cross-module RPC
run(M, fail(A), [fail(A?)]).                     % report failure
run(M, A, Xs?) :- tuple(A?) |                    % reduce
    M # reduce(A?, B), run(M?, B?, Xs).
\end{verbatim}
...
\begin{verbatim}
run_counted(M, true, 0).
run_counted(M, (A,B), N?) :-
    run_counted(M?, A?, NA), run_counted(M?, B?, NB),
    N := NA? + NB?.
run_counted(M, M1 # G, N?) :- run_counted(M1?, G?, N).
run_counted(M, A, N?) :- tuple(A?) |
    M # reduce(A?, B), run_counted(M?, B?, NB), N := NB? + 1.
\end{verbatim}
...
run(M, (A,B), (TA?,TB?)) :- run(M?,A?,TA), run(M?,B?,TB).  % fork
run(M, M1 # G, T?) :- run(M1?, G?, T).                 % cross-module RPC
run(M, A, ((I?:Time?):-TB?)) :- tuple(A?) |
    time(Time), M # reduce(A?, B, I), run(M?, B?, TB).
\end{verbatim}
...
A controller creates an equator, keeps the handle (sole reader), and passes the structure to the meta-interpreter:
...
The meta-interpreter distributes the equator when forking:
...
The controller detects the signal via the sole reader:
\begin{verbatim}
controller(Handle, Commands) :-
    known(Handle?) |
    issue_stop(Commands?).
