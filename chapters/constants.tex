...
The trace shows how values flow through the clause. In the head, \verb|A| and \verb|B| are writers that receive the goal constants \verb|1| and \verb|1|. The reader \verb|Z?| pairs with the goal's writer \verb|X|, establishing a channel. In the body, \verb|A?| and \verb|B?| read the bound values, passing them to \verb|and|. The intermediate variable \verb|W| connects \verb|and|'s output to \verb|not|'s input. Finally, \verb|Z| writes the result, which flows through the channel to bind \verb|X|.
...
The \verb|ground| guard ensures both inputs are bound before committing. Without it, the SRSW discipline would be violated: \verb|A?| appears in both \verb|xor| and \verb|and|, meaning two readers would share the same writer. The guard suspends until the values arrive, then commits, at which point the bound values can safely be read multiple times.
...
The trace reveals the suspension mechanism. When \verb|half_adder(X1?,1,X3,X4)| is spawned, \verb|X1?| is a reader whose paired writer is not yet bound. The goal suspends, waiting for data. Meanwhile, the first half-adder computes and binds \verb|X1| to \verb|0|. This binding reactivates the suspended goal, which now proceeds with concrete values. Similarly, \verb|or| suspends twice---first waiting for both carries, then waiting for the second carry---resuming each time a value arrives.
