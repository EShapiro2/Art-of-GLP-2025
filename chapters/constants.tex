...
The parameter conventions in \verb|nand| follow necessarily from writer unification
(Definition~\ref{definition:GLP-unification}). Understanding this derivation prevents
common errors.

\mypara{Output Parameters}
Consider the third argument of \verb|nand(A,B,Z?) :- and(A?,B?,W), not(W?,Z).|

When the goal \verb|nand(1,1,X)| unifies with the head, the third argument position
has goal writer \verb|X| against head reader \verb|Z?|. From the unification catalog
...
unification: only writers can be assigned, and goal writers can receive values
