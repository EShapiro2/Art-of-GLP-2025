...
\begin{remark}[Writer-to-Writer and Suspension]
By the formal definition, unifying two writers $X$ and $Y$ would suspend on an empty set: the regular mgu $\{X := Y\}$ exists but is not a writer mgu (violating condition 2), and the suspension set $W_\sigma = \emptyset$ since no reader is mapped to a non-variable. However, suspending on an empty set means waiting for nothing—guaranteed deadlock. By SRSW, after binding $X$ to $Y$, no other occurrence of either writer remains to instantiate them, so their paired readers $X?$ and $Y?$ are abandoned. The implementation therefore fails immediately on writer-to-writer unification rather than suspending uselessly. This is a pragmatic extension: early detection of inevitable deadlock.
\end{remark}
...
A key simplification compared to the GLP operational semantics is the immediate application of reader substitutions during reduction rather than through asynchronous communication. This is appropriate for workstation execution where all variables are local.
