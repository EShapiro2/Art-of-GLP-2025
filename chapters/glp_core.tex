...
The following table enumerates all cases of writer unification between a goal
argument and a clause head argument. In the table, $c$ and $d$ denote distinct
constants, $X$ denotes a goal writer, $X?$ denotes a goal reader, $V$ denotes a
head writer (fresh after renaming), and $V?$ denotes a head reader.

\begin{center}
\begin{tabular}{|c|c|l|}
\hline
\textbf{Head} & \textbf{Goal} & \textbf{Writer Unification Result} \\
\hline
$c$ & $c$ & succeeds (identical constants) \\
$c$ & $d$ & fails (distinct constants) \\
$c$ & $X$ & succeeds: $\{X \mapsto c\}$ \\
$c$ & $X?$ & suspends on $\{X?\}$ \\
$V$ & $c$ & succeeds: $\{V \mapsto c\}$ \\
$V$ & $X$ & succeeds: $\{V \mapsto X\}$ \\
$V$ & $X?$ & succeeds: $\{V \mapsto X?\}$ \\
$V?$ & $c$ & succeeds: $\{V \mapsto c\}$ \\
$V?$ & $X$ & succeeds: $\{V \mapsto X\}$ \\
$V?$ & $X?$ & succeeds: $\{V \mapsto X?\}$ \\
...
Each row shows what happens when a single argument position is unified. The head
column shows what appears in the clause head after renaming apart (fresh variables
$V$, $V?$). The goal column shows what appears in the goal being reduced.

Key observations:
\begin{itemize}
\item A goal writer $X$ can always be assigned---it succeeds against any head form.
\item A goal reader $X?$ suspends when matched against a non-variable (rows 4),
      because readers cannot be assigned directly; the paired writer must be
      assigned first.
\item Head readers $V?$ behave like head writers $V$ for unification purposes,
      because the fresh variable $V$ can be assigned. The distinction matters
      for the clause body, where $V?$ will be read.
\end{itemize}

Note that the table shows outcomes for individual argument positions. For goals with multiple arguments, the two-phase algorithm (collection then resolution) determines the overall outcome. A reader that initially appears to require suspension may be resolved by a writer binding from a later argument position.
\end{remark}
...
In any $GLP$ run $G_0 \rightarrow G_1 \rightarrow \cdots$, if atom $A \in G_i$ can reduce with clause $C$, then for any $j > i$, either $A$ has been reduced by step $j$, or there exists $A' \in G_j$ where $A' = A\tau$ for some reader assignment $\tau$, and $A'$ can reduce with $C$.
\end{restatable}
