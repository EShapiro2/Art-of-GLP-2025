...
Circular terms may form through cross-goal communication. For example, given clause \verb|p(X?,X)| and goals \verb|p(X,f(Y?)), p(Y,f(X?))|, after both reductions \verb|X| becomes \verb|f(f(X?)))|---circular. In a multiagent setting, these goals may execute on different agents, making any occurs check infeasible. Implementations must handle circular terms gracefully.
