...
The guard \verb|Xs =\= []| (Appendix~\ref{appendix:guard-unification}) is an inequality test that succeeds when \verb|Xs| is bound to a non-empty list, suspends when \verb|Xs| is unbound, and fails when \verb|Xs| is the empty list. This ensures the recursive clause only commits when there is at least one element to process.
