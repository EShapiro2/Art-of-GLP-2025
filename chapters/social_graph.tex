...
The protocols presented above describe agent behavior in isolation. To verify that multiple agents interact correctly, we simulate multiagent systems within GLPSAM. This section presents the \emph{play} methodology: self-contained programs that spawn multiple agents, connect them through a network switch, and drive their behavior through scripted actors.
...
The GLP paper specifies the switch behavior with defined guards:

\begin{verbatim}
% Representative clause for forwarding from p to q:
network(...,(p,Chp),...,(q,Chq),.... :-
...
For GLPSAM execution, we unfold the \verb|send| and \verb|receive| guards. A channel \verb|ch(In?, Out)| pairs an input stream (which the holder reads) with an output stream (which the holder writes). The network holds the inverse of each agent's channel: where Alice has \verb|ch(NetIn?, NetOut)|, the network has \verb|ch(NetOut?, NetIn)|.

Unfolding the guards yields the GLPSAM form. For a two-agent network:
...
\subsection{GLPSAM Adaptations}

The GLPSAM versions of protocol predicates differ from the deployment versions in two ways:

\paragraph{Guard unfolding.} Defined guards like \verb|send|, \verb|receive|, and \verb|new_channel| are manually unfolded. For example, the deployment version:
...
The guard unfolds to direct construction of the channel pair with appropriate reader/writer annotations.

\paragraph{Attestation omission.} The \verb|attestation/2| guard, which verifies cryptographic signatures in deployment, is simply removed. All agents are assumed correct, and all messages authentic.

These adaptations are mechanical and preserve protocol logic. Code that works in GLPSAM simulation will work in deployment once the runtime implements guard evaluation and attestation verification.
