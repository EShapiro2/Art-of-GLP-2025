...
The initialization pattern-matches the channel structures directly in the head, avoiding guard-based unification that would violate SRSW.  The input streams are merged into a unified stream \verb|In|, and output streams are stored in the initial friends list with special identifiers \verb|user| and \verb|net|.
...
On acceptance, the predicate constructs a channel pair directly in the head: \verb|ch(FOut?, FIn)| goes to the remote agent (via the response variable), while \verb|ch(FIn?, FOut)| is kept locally. The reader/writer annotations ensure each stream has exactly one writer.
...
The channel pair is constructed directly: \verb|PtoQ| and \verb|QtoP| are fresh variables that form the two halves. P receives \verb|ch(QtoP?, PtoQ)| (reads from Q, writes to Q), and Q receives \verb|ch(PtoQ?, QtoP)| (reads from P, writes to P).
...
\label{sec:channel-notation}

A \emph{channel} \verb|ch(In, Out)| pairs an input stream (which the holder reads) with an output stream (which the holder writes). The following notation describes channel operations semantically:

\begin{itemize}
\item \textbf{Send:} Adding message \verb|X| to channel \verb|ch(In, Out)| yields \verb|ch(In, [X|Out'])|, extending the output stream.
\item \textbf{Receive:} Reading from channel \verb|ch([X|In'], Out)| yields message \verb|X| and continuation \verb|ch(In', Out)|.
\item \textbf{New channel:} A fresh channel pair consists of \verb|ch(Xs, Ys)| and \verb|ch(Ys, Xs)|---each channel's input is the other's output.
\end{itemize}

In code, these operations are expressed through direct stream manipulation in clause heads, as shown in the protocol clauses above.
...
tag_stream(Name, [M|In], [msg(Name?, M?)|Out?]) :-
    constant(Name?) | tag_stream(Name?, In?, Out).
tag_stream(_, [], []).
\end{verbatim}
