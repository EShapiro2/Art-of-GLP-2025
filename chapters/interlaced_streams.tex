...
\begin{itemize}
\item If \verb|Bs| is an unbound variable (\verb|unknown(Bs?)| succeeds), then \verb|Block| is the current tip---the most recent block not yet extended. Include it in the tips list.
\item Otherwise (\verb|Bs| is bound), \verb|Block| has already been extended. Skip it and continue with the tail.
\end{itemize}

The \verb|unknown(X?)| guard is the key mechanism: it succeeds when \verb|X| is an unbound variable. This detects stream positions that have not yet been extended by their owner.

\section{The unknown Guard}
\label{sec:unknown-guard}

The \verb|unknown(X?)| guard tests whether a variable is unbound:
\begin{itemize}
\item \textbf{Succeeds} if \verb|X?| is an unbound variable
\item \textbf{Fails} if \verb|X?| is bound to any value
\end{itemize}
