...
Multiple input streams can be merged into a single output stream. Merging is fundamental to concurrent programming: it lets multiple producers contribute to a single consumer.

\subsection{Simple Fair Merge}

The basic merge alternates between two input streams:
...
\begin{verbatim}
GLP> merge([1,2], [a,b], Out).
merge([1, 2], [a, b], Out) :- merge([a, b], [2], X1)
merge([a, b], [2], X1) :- merge([2], [b], X2)
merge([2], [b], X2) :- merge([b], [], X3)
merge([b], [], X3) :- merge([], [], X4)
merge([], [], X4) :- true
Out = [1, a, 2, b]
→ succeeds
...
\begin{remark}[Biased Merge]
This merge is \emph{biased}: when both streams have elements ready, it always takes from the first. True fairness requires nondeterminism or external arbitration. For many applications, bias is acceptable---the key property is that neither stream is starved indefinitely.
\end{remark}

\subsection{Dynamic Merge}

Often we don't know in advance how many streams to merge. A \emph{dynamic merge} accepts new streams at runtime:
...
Each \verb|add(S)| message creates a new merge node combining stream \verb|S| with the accumulated input.

\begin{verbatim}
GLP> dynamic_merge([add([1,2]), add([a,b])], [], Out).
% [PLACEHOLDER: Insert actual trace here]
Out = [a, 1, b, 2]
→ succeeds
\end{verbatim}
...
Dynamic merge builds a tree structure, but the shape depends on arrival order:

\begin{verbatim}
Control: [add(S1), add(S2), add(S3)]
...
This creates a \emph{linear chain}: S1 passes through two merge nodes to reach the output, while S3 passes through one. Streams added earlier suffer higher latency.

Worse, with $N$ streams the leftmost stream has $O(N)$ latency. For fair treatment, we want $O(\log N)$---a balanced tree.
...
\begin{enumerate}
\item \textbf{Rebalancing}: Periodically restructure the merge tree. Complex and may lose elements during restructuring.

\item \textbf{Round-robin at leaves}: Instead of tree merge, use a single coordinator that polls streams in rotation. Simpler but creates a bottleneck.

\item \textbf{Static balanced tree}: If the maximum number of streams is known, pre-build a balanced tree and connect streams as they arrive.

\item \textbf{Weighted merge}: Give recently-added streams priority to compensate for their shorter path. Approximates fairness without restructuring.
\end{enumerate}

For most applications, the static balanced approach offers the best tradeoff between fairness and complexity.

\subsection{Static Balanced Merge Tree}

Given a list of streams, build a balanced merge tree by pairing streams layer by layer:
...
The \verb|merge_tree| predicate builds a balanced tree bottom-up: \verb|merge_layer| pairs adjacent streams with \verb|merge|, halving the count. This repeats until one stream remains.

For 4 streams \verb|[[a,b], [1,2], [x,y], [p,q]]|:
\begin{verbatim}
          Out
           |
        merge
...
Every stream passes through exactly $\lceil \log_2 N \rceil$ merge nodes, ensuring fair latency.
...
\item \label{ex:biased-merge} Write \verb|biased_merge(Xs, Ys, Zs)| that takes \emph{all} available elements from \verb|Xs| before taking from \verb|Ys|. That is, it takes from \verb|Ys| only when \verb|Xs| suspends (its tail is unbound). Hint: use the \verb|writer| guard. (Solution in Appendix~\ref{app:solutions}.)
\end{enumerate}
