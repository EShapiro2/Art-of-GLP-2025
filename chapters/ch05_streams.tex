...
Given $N$ potential streams, build a complete binary tree of merge nodes:

\begin{verbatim}
% Build balanced merge tree for N streams
% Returns list of input slots and final output stream
build_tree(1, [Out?], Out).
build_tree(N, Slots?, Out) :- N? > 1 |
    Half := N? // 2,
    Rest := N? - Half?,
    build_tree(Half?, Left, L),
    build_tree(Rest?, Right, R),
    merge(L?, R?, Out?),
    append(Left?, Right?, Slots).
\end{verbatim}
...
Every stream passes through exactly $\lceil \log_2 N \rceil$ merge nodes.
