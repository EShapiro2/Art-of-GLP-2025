...
The logarithmic delay of balanced merge trees can be eliminated entirely using \emph{mutual references}---a mechanism for multiple processes to share access to a single output stream endpoint.

\paragraph{The Problem with Tree-Based Merges.}

In any tree-based merge, a message from an input stream must traverse the tree to reach the root. Even in a balanced tree with $n$ inputs, each message requires $O(\log n)$ reductions. For $m$ total messages, this gives $O(m \log n)$ total work.

Can we do better? If all $n$ streams could directly append to the same output stream, each append would be $O(1)$, giving $O(m)$ total work regardless of $n$.

\paragraph{Mutual References.}

A \emph{mutual reference} is a mutable pointer that can be shared among multiple processes. In GLP, it points to the current tail of an output stream. The key operation is \emph{atomic append}: a process atomically reads the current tail, extends the stream with a new element, and updates the pointer to the new tail.

The system predicate \verb|mwm(Streams, Out)| creates a multiway merge using mutual references:

\begin{verbatim}
mwm(Streams, Out?) :-
    mr_create(Tail, MR),      % Create mutual reference to tail
    Out = [started?|Tail?],    % Initialize output with started marker
    mwm1(Streams?, MR?).       % Start copy processes
\end{verbatim}

The predicate \verb|mr_create(Tail, MR)| creates a mutual reference \verb|MR| initialized to point to \verb|Tail|. The output stream begins with a \verb|started| marker, followed by the tail that will be extended by the copy processes.

\paragraph{Copy Processes.}

Each input stream gets a copy process that forwards messages to the shared output:
...
The guard \verb|is_mutual_ref(MR?)| verifies that \verb|MR| is a valid mutual reference. This is necessary because \verb|MR?| appears multiple times---once for each copy process. Unlike \verb|ground|, which allows replication of immutable values, \verb|is_mutual_ref| allows sharing of a mutable reference.

Each copy process reads from its input and appends to the shared output:

\begin{verbatim}
mwm_copy([X|Xs], MR?) :-
    is_mutual_ref(MR?) |
    stream_append(MR?, X?),    % Atomic append to output
    mwm_copy(Xs?, MR?).
mwm_copy([], MR?) :-
    stream_append(MR?, halted?).  % Signal this stream ended
\end{verbatim}

The \verb|stream_append(MR, X)| operation atomically:
\begin{enumerate}
\item Reads the current tail from \verb|MR|
\item Binds that tail to \verb|[X|NewTail]|
\item Updates \verb|MR| to point to \verb|NewTail|
\end{enumerate}

All copy processes race to append, but the atomic operation ensures no messages are lost or duplicated.

\paragraph{Termination Detection.}

When all input streams close, the output should close too. The \verb|halted| markers enable termination detection:

\begin{verbatim}
close_when_done(0, MR?) :-
    mr_close(MR?).             % Bind tail to []
close_when_done(N, MR?) :- N? > 0 |
    mr_wait_halted(MR?),       % Wait for a halted marker
    N1 := N? - 1,
    close_when_done(N1?, MR?).
\end{verbatim}

The process starts with \verb|N| equal to the number of input streams. Each \verb|halted| marker decrements the count. When all streams have halted, \verb|mr_close| binds the output tail to \verb|[]|.

\paragraph{Complexity Analysis.}

With mutual references:
\begin{itemize}
\item Each message append is $O(1)$---no tree traversal
\item Total work for $m$ messages from $n$ streams: $O(m + n)$
\item The $+n$ accounts for spawning copy processes and termination detection
\end{itemize}

Compare to balanced merge trees: $O(m \log n)$ total work. For large $n$ with many messages per stream, the constant-delay approach is significantly more efficient.

\paragraph{Trade-offs.}

Mutual references introduce shared mutable state, which has implications:
\begin{itemize}
\item \textbf{Atomicity overhead}: The \verb|stream_append| operation requires synchronization
\item \textbf{Fairness}: Unlike binary merge which guarantees alternation, mutual reference merge provides no ordering guarantees among inputs
\item \textbf{Distributed systems}: Mutual references work within a single agent; distributed multiway merge requires different techniques (see Part II)
\end{itemize}

For a detailed treatment of multiway merge and its complexity analysis, see Shapiro and Safra~\cite{shapiro1986multiway}.
