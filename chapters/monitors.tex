...
\subsection{Four-Actor Play}

A more complex play demonstrates multiple actors and coordination patterns:

\begin{verbatim}
%% A play with four actors: Director, Writer, Editor, Publisher
%% Director initiates and coordinates, passing control around

director([begin|Tail?], Result?) :-
    writer(Tail, Result).

writer([draft(chapter1), draft(chapter2)|Tail?], Result?) :-
    editor(Tail, Result).

editor([edit(chapter1), edit(chapter2)|Tail?], Result?) :-
    publisher(Tail, Result).

publisher([publish|Tail?], Result?) :-
    director_finish(Tail, Result).

director_finish([complete], finished).
\end{verbatim}

The four actors create a document production workflow:

\begin{verbatim}
GLP> director(Stream, Status).
director(X1, X2) :- writer(X3, X4)
writer(X3, X4) :- editor(X5, X6)
editor(X5, X6) :- publisher(X7, X8)
publisher(X7, X8) :- director_finish(X9, X10)
director_finish(X9, X10) :- true
Stream = [begin, draft(chapter1), draft(chapter2),
          edit(chapter1), edit(chapter2), publish, complete]
Status = finished
→ succeeds
\end{verbatim}
