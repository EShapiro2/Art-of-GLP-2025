...
\mypara{Type Guards}
Type guards like \verb|string(X?)|, \verb|number(X?)|, and \verb|integer(X?)|
also enable multiple reader occurrences. A string or number cannot contain
...
\verb|string(X?)|, \verb|number(X?)| & Yes (strings/numbers have no variables) \\
...
alice([msg(hello, Reply)|Rest]) :-
    string(Reply?) |       % Waits for Reply to be bound
    process(Reply?),     % Process the response
    alice(Rest?).
\end{verbatim}

This deadlocks. The guard \verb|string(Reply?)| waits for \verb|Reply| to be
...
alice_wait(Reply, Rest) :-
    string(Reply?) |       % Now waiting for bob's response
    process(Reply?),
    alice(Rest?).
\end{verbatim}
