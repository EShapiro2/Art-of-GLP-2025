...
The type guards (\verb|tuple|, \verb|constant|) distinguish regular messages from \verb|merge| requests. For tuples, the additional guard \verb|~(X? =?= merge(_))| ensures we don't accidentally match a merge request. Splitting clauses by type is SRSW-compliant: type guards permit multiple reader occurrences (per Formal Box~\ref{formal:guards-multiple-reads}), so \verb|X?| can appear both in the output and in the type test.
...
This technique relies on writer-to-writer term matching being \emph{immediate failure}, not suspension. If WxW suspended on an empty set, clause 2 would never be tried. The observer pattern demonstrates that WxW failure is semantically meaningful for clause selection, not merely an optimization.
...
Difference lists are a programming technique inherited from Prolog and FCP that enables efficient list construction and concatenation. In GLP, difference lists require explicit mode annotations that reflect their data flow semantics, making the technique both more disciplined and more transparent than in earlier logic programming languages.

\subsubsection{The Concept}

A difference list represents a list with a ``hole'' at the end---a position where additional elements can be appended. Rather than representing a complete list like \texttt{[a,b,c]}, a difference list represents the \emph{difference} between a list and its tail. The list \texttt{[a,b,c]} can be represented as the difference between \texttt{[a,b,c|T]} and \texttt{T}, where \texttt{T} is an unbound variable serving as the hole.

The key advantage of difference lists is that concatenation becomes a constant-time operation. To append two difference lists, we simply connect the hole of the first to the front of the second. This contrasts with ordinary list append, which requires traversing the entire first list.

\subsubsection{Notation}

GLP uses the notation \texttt{H\textbackslash T} to denote a difference list, where \texttt{H} is the list structure (the ``front'') and \texttt{T} is the hole reference (the ``back''). This notation makes difference lists visually distinct from ordinary lists and emphasizes their two-component nature.

The list \texttt{[a,b,c]} can be represented as a difference list in several equivalent ways:
\begin{verbatim}
[a,b,c|T]\T          % List [a,b,c] with hole T
[a,b|T]\T            % List [a,b] with hole T
[a|T]\T              % List [a] with hole T
T\T                  % Empty difference list
\end{verbatim}

\subsubsection{The Mode Invariant}

In GLP, difference lists obey a fundamental mode invariant: the two components always have complementary modes. A difference list is either \texttt{H?\textbackslash T} (reader head, writer tail) or \texttt{H\textbackslash T?} (writer head, reader tail), but never \texttt{H?\textbackslash T?} or \texttt{H\textbackslash T}.

\begin{formal}{Difference List Mode Invariant}
A well-formed difference list \texttt{H\textbackslash T} in a GLP clause satisfies exactly one of:
\begin{enumerate}
\item \texttt{H?\textbackslash T} --- The head is a reader (consuming the list structure) and the tail is a writer (providing access to the hole).
\item \texttt{H\textbackslash T?} --- The head is a writer (constructing the list structure) and the tail is a reader (receiving the hole position).
\end{enumerate}
This invariant extends to partially instantiated forms: \texttt{[A?,B?|H]\textbackslash T?} and \texttt{H?\textbackslash [\_|T]} preserve the complementary mode relationship.
\end{formal}

The mode invariant directly reflects data flow. When a difference list is being constructed, the head grows (writer) while the tail remains a reference point (reader). When consumed, the head is traversed (reader) while the tail may be connected elsewhere (writer).

\subsubsection{SRSW Compliance}

The mode invariant ensures that difference list programs naturally satisfy SRSW. Consider difference list append:

\begin{program}{Difference List Append}
\begin{verbatim}
dl_append(H1\Xs?, Xs\T2?, H1?\T2).
\end{verbatim}
\end{program}

Tracing SRSW compliance: \texttt{Xs} has one writer (in \texttt{Xs\textbackslash T2?}) and one reader (in \texttt{H1\textbackslash Xs?}). \texttt{H1} has one writer (in \texttt{H1\textbackslash Xs?}) and one reader (in \texttt{H1?\textbackslash T2}). \texttt{T2} has one writer (in \texttt{H1?\textbackslash T2}) and one reader (in \texttt{Xs\textbackslash T2?}). Each variable appears exactly once as writer and once as reader.

\subsubsection{Comparison with FCP}

In FCP, difference list programs rely on implicit unification:
\begin{verbatim}
%% FCP style (implicit unification)
append_dl(H1-T1, T1-T2, H1-T2).
\end{verbatim}

The variable \texttt{T1} appears three times; unification handles the connection implicitly. In GLP, explicit modes make data flow visible:
\begin{verbatim}
%% GLP style (explicit modes)
dl_append(H1\Xs?, Xs\T2?, H1?\T2).
\end{verbatim}

The annotations show that \texttt{Xs} is provided by the second argument (writer) and consumed by the first (reader). This explicitness aids understanding and enables compile-time SRSW verification.
...
The \verb|--| operator is simply a binary functor with no special meaning---we use it to pair the read and write ends of the buffer. When the goal \verb|receive(I, Buf?, NBuf)| unifies with the head \verb|receive(Msg, [Msg|NBuf]--[_|NTail?], NBuf--NTail)|, the second argument demonstrates compound term matching (Appendix~\ref{appendix:compound-unification}): the goal's \verb|Buf?| must match the structure \verb|[...]--[...]|. If \verb|Buf?| is unbound (its paired writer not yet assigned), unification suspends. When \verb|Buf| receives a difference-list value, the goal reactivates and unification proceeds by matching the \verb|--| functor and recursively unifying its arguments.
...
The \verb|read(C?)| pattern uses the state reader directly in the message structure. The client sends \verb|read(V)| retaining reader \verb|V?|; when the clause head matches, pattern matching binds the client's writer \verb|V| to the current count \verb|C?|. The \verb|number(C?)| guard permits replication of \verb|C?| to both the message and the recursive call (per Formal Box~\ref{formal:guards-multiple-reads}).
...
\mypara{Type Guards}
Type guards like \verb|constant(X?)| and \verb|integer(X?)|
also enable multiple reader occurrences. A constant cannot contain
...
From a term matching perspective, when the monitor's clause head \verb|counter_loop([read(V)|In], C)| matches a goal, the \verb|V| in the message is a fresh writer (renamed from the clause). The body assignment \verb|V = C?| binds this writer to the current count. The induced reader assignment then propagates this value through the communication channel: the client's reader \verb|V?| receives the count.
...
alice([msg(hello, Reply)|Rest]) :-
    constant(Reply?) |       % Waits for Reply to be bound
    process(Reply?),     % Process the response
    alice(Rest?).
\end{verbatim}

This deadlocks. The guard \verb|constant(Reply?)| waits for \verb|Reply| to be
...
alice_wait(Reply, Rest) :-
    constant(Reply?) |       % Now waiting for bob's response
    process(Reply?),
    alice(Rest?).
\end{verbatim}
...
\item[$\star$] \textbf{Count-Up Producer.} The text shows a countdown producer. Implement a count-up producer that generates an infinite stream $[0, 1, 2, 3, \ldots]$:
\begin{verbatim}
producer(Xs) :- producer(Xs?, 0).
producer([N?|Xs], N) :- N1 := N? + 1, producer(Xs, N1?).
\end{verbatim}
Explain why this producer never terminates and why the consumer must not have a clause for \verb|[]|.
...
\item \textbf{Parallel Lookup Table.} Implement a parallel lookup table using channels. Each \verb|table| process manages a key range and handles \verb|lookup| and \verb|store| requests:
\begin{verbatim}
table(Self, LowKey, Value, HighKey) :-
    read(lookup(LowKey, Value?, true), Self?, NewSelf) |
    table(NewSelf?, LowKey?, Value?, HighKey?).
\end{verbatim}
Extend with clauses for: (1) lookup returning \verb|false| when key is in range but not exact match, (2) store that splits the table segment when inserting a new key.

\item \textbf{Binary Address Routing.} Implement \verb|distribute_binary(In,Out1,Out2)| that routes messages based on binary address bits. Messages have the form \verb|req(Addr,Content)| where \verb|Addr| is a list of 0s and 1s. The first bit determines left (0) or right (1) routing; the remaining bits are passed to the subtree:
\begin{verbatim}
distribute_binary([req([0|Addr],C)|In], [req(Addr?,C?)|Out1?], Out2?) :-
    distribute_binary(In?, Out1, Out2).
\end{verbatim}
Complete the definition and trace \verb|distribute_binary([req([0],a), req([1],b)], Y, Z)|.
\end{enumerate}
...
\item \textbf{2x2 Communication Switch.} Implement \verb|switch2x2(In1,In2,Out1,Out2)| that routes messages from two input ports to two output ports. Each clause receives from one input and sends to one output, with four clauses covering all combinations:
\begin{verbatim}
switch2x2(In1, In2, Out1, Out2) :-
    receive(M, In1?, Ins1), send(M?, Out1?, Outs1) |
    switch2x2(Ins1?, In2?, Outs1?, Out2?).
\end{verbatim}
What determines which routing path is chosen? How does the switch achieve fair interleaving?
\end{enumerate}
...
\item \textbf{Absolute Time Scheduling.} Extend the multi-actor monitor scenario to use absolute time targets instead of relative delays. Compute wait deltas from absolute targets:
\begin{verbatim}
bob_at(T0, TTarget, V, Ys?) :-
    ...
    bob_done(Ys1).
\end{verbatim}
Implement \verb|play_absolute| with four actors scheduled at $T_0$, $T_0+50$, $T_0+100$, and $T_0+150$ milliseconds.

\item \textbf{Dynamic Object Management.} Implement \verb|use_many_counters(Input,List)| that dynamically creates and manages multiple counter objects. Handle three message types: \verb|create(Name)| spawns a new counter, \verb|(Name,Cmd)| routes commands to named counters, and \verb|(Name,show(Val))| retrieves a counter's value:
\begin{verbatim}
use_many_counters([create(Name)|Input?], List_of_counters) :-
    counter(Com?, 0),
    use_many_counters(Input?, [(Name?, Com)|List_of_counters?]).
\end{verbatim}
Complete the definition including the \verb|send| helper for routing messages.

\item \textbf{3-Way Network Switch.} Extend the 2-way network switch to three agents $p$, $q$, $r$. Each agent sends messages of the form \verb|msg(Dest,Content)| where \verb|Dest| names the recipient. The switch examines the destination and forwards to the appropriate channel:
\begin{verbatim}
network((P,ChP),(Q,ChQ),(R,ChR)) :-
    ground(Q), receive(ChP?,msg(Q,X),ChP1), send(ChQ?,X?,ChQ1) |
    network((P,ChP1?),(Q,ChQ1?),(R,ChR?)).
\end{verbatim}
How many clauses are needed for complete routing? What is the purpose of \verb|ground(Q)| in the guard?
\end{enumerate}
